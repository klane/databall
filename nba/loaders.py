...
class GameLoader(ItemLoader):
    ...
    location_in = MapCompose(unicode.strip, lambda x: x if len(x) > 0 else u'vs')
    result_in = MapCompose(unicode.strip, unicode.split)
    ...
    spread_result_in = MapCompose(unicode.strip, unicode.split, lambda x: x if x != u'-' else None)
